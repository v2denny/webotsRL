...
class WebotsEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(
            low=np.array([0] * 8 + [0, -np.pi]),  # 8 lidar sectors, distance, angle
            high=np.array([1] * 8 + [1, np.pi]),  # Adjust high values as needed
            dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        reward, done = self.calculate_reward_done_v4()
    ...
    def calculate_reward_done_v4(self):
        self.supervisor.step()
        ...
        reward += 10 * (self.prev_distance_to_target - distance)  # Reward for getting closer
    ...
    def process_lidar(self, point_cloud):
        ...
        #####
        min_distances = np.clip(min_distances / 2, 0, 1)  # Normalize to range [0, 1]
...
env = WebotsEnv()
model_path = 'none'
checkpoint_callback = CheckpointCallback(save_freq=50000, save_path='./', name_prefix='ppov3')
